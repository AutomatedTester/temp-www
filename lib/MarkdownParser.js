...
module.exports = class Parser {
  ...
  async getReleases() {
    ...
    const template = this.content['_sections_']._releases;
    try {
      ...
      this.content.guide.overview['whats-new'].content = ejs.render(template, {
        releases
      });
    } catch (err) {
      ...
      if (Array.isArray(subsections)) {
        ...
        Object.keys(subsections).forEach(subName => {
          ...
          subPages.forEach((subPageName, index, total) => {
            ...
            pageContent += `<div class="improve-article-bottom" data-github-link="${githubLink}" style="display: none"></div>`;
            ...
            if (metadata && metadata['summary\\_image']) {
              metadata['summary_image'] = metadata['summary\\_image'];
            }
